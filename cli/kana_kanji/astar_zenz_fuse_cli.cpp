...
static kk::YomiSearchMode parse_yomi_mode(const std::string &s) {
    ...
    if (s == "cps_pred") return kk::YomiSearchMode::CommonPrefixPlusPredictive;
    if (s == "cps_omit") return kk::YomiSearchMode::CommonPrefixPlusOmission;
    if (s == "all")      return kk::YomiSearchMode::All;
...
static std::string hira_to_kata_utf8(const std::string & utf8) {
    ...
    while (i < n) {
        ...
        if (0x3041 <= cp && cp <= 0x3096) cp += 0x60;
        append_u8(cp);
        i += adv;
    }
...
static const float * get_logits_kv(
    ...
    if (need <= 0) return llama_get_logits(ctx);
...
public:
    ...
    explicit ZenzRunner(const Options & opt) : opt_(opt) {
        if (opt_.threads <= 0) opt_.threads = default_threads();
...
static ZenzMode parse_zenz_mode(const std::string & s) {
    if (s == "off")      return ZenzMode::Off;
    if (s == "gen")      return ZenzMode::Gen;
    if (s == "eval")     return ZenzMode::Eval;
    if (s == "gen_eval") return ZenzMode::GenEval;
...
static void run_one_fused(
    ...
    if (zenzMode == ZenzMode::Eval || zenzMode == ZenzMode::GenEval) {
        ...
        if (best_pass >= 0) {
            ...
            for (int i : idx) sorted.push_back(std::move(out[(size_t)i]));
            out = std::move(sorted);

            if (zenzShowEval) {
                std::vector<ZenzRunner::EvalOut> es;
                es.reserve(evals.size());
                for (int i : idx) es.push_back(std::move(evals[(size_t)i]));
                evals = std::move(es);
            }
        }
    }
    ...
    switch (zenzMode) {
        case ZenzMode::Off:     std::cout << "off"; break;
        case ZenzMode::Gen:     std::cout << "gen"; break;
        case ZenzMode::Eval:    std::cout << "eval"; break;
        case ZenzMode::GenEval: std::cout << "gen_eval"; break;
    }
...
int main(int argc, char ** argv) {
    ...
    bool showYomi = false;
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            // A* paths
            if (a == "--yomi_termid") { yomi_termid_path = need("--yomi_termid"); continue; }
            if (a == "--tango")       { tango_path       = need("--tango");       continue; }
            if (a == "--tokens")      { tokens_path      = need("--tokens");      continue; }
            if (a == "--pos_table")   { pos_path         = need("--pos_table");   continue; }
            if (a == "--conn")        { conn_path        = need("--conn");        continue; }

            // query
            if (a == "--q")      { q = need("--q"); continue; }
            if (a == "--stdin")  { stdin_mode = true; continue; }

            // A* knobs
            if (a == "--n")      { nBest = std::stoi(need("--n")); continue; }
            if (a == "--beam")   { beamWidth = std::stoi(need("--beam")); continue; }
            if (a == "--show_bunsetsu") { showBunsetsu = true; continue; }
            if (a == "--yomi_mode") { yomi_mode_str = need("--yomi_mode"); continue; }
            if (a == "--pred_k") { predK = std::stoi(need("--pred_k")); continue; }
            ...
            // Zenz knobs
            if (a == "--zenz_left")        { zopt.left        = need("--zenz_left"); continue; }
            if (a == "--zenz_profile")     { zopt.profile     = need("--zenz_profile"); continue; }
            if (a == "--zenz_topic")       { zopt.topic       = need("--zenz_topic"); continue; }
            if (a == "--zenz_style")       { zopt.style       = need("--zenz_style"); continue; }
            if (a == "--zenz_preference")  { zopt.preference  = need("--zenz_preference"); continue; }

            if (a == "--zenz_n_ctx")       { zopt.n_ctx       = std::stoi(need("--zenz_n_ctx")); continue; }
            if (a == "--zenz_n_batch")     { zopt.n_batch     = std::stoi(need("--zenz_n_batch")); continue; }
            if (a == "--zenz_threads")     { zopt.threads     = std::stoi(need("--zenz_threads")); continue; }
            if (a == "--zenz_gpu_layers")  { zopt.gpu_layers  = std::stoi(need("--zenz_gpu_layers")); continue; }
            if (a == "--zenz_no_mmap")     { zopt.use_mmap    = false; continue; }
            if (a == "--zenz_offload_kqv") { zopt.offload_kqv = true; continue; }

            if (a == "--zenz_max_new")     { zopt.max_new     = std::stoi(need("--zenz_max_new")); continue; }
            if (a == "--zenz_min_len")     { zopt.min_len     = std::stoi(need("--zenz_min_len")); continue; }

            if (a == "--verbose") { verbose = true; zopt.verbose = true; continue; }
